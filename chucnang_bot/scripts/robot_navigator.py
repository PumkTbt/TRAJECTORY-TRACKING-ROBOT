...
# Quy uoc cac gia tri tra ve trang thai hoat dong cua node navigation 
...
class BasicNavigator(Node):
    ...
    def isNavComplete(self):
        if not self.result_future:
            return True     # Nhiem vu da bi huy hoac da hoan thanh xong
        ...
        if self.result_future.result():
            ...
            return False      # He thong bi treo hoac chưa hoan thanh nhiem vu

        self.debug('Goal succeeded!') # Da hoan thanh xong quang duong di chuyen
        return True
...
# ---------------------------------------------------------------------------
        ...
        for i in range(len(self.result_future.result().result.path)):
            pose = PoseStamped()
            pose.pose.position.x = self.result_future.result().result.path[i][0]
            pose.pose.position.y = self.result_future.result().result.path[i][1]
        ...
        for srv in srvs:
            if srv[1][0] == 'nav2_msgs/srv/ManageLifecycleNodes':
                ...
                future = mgr_client.call_async(req)
                 # ----------------------- Bat dau voi  viec xac dinh toa do vi tri hien tai cua robot tren map -----------------------------------
                while True:   
...
# ------------ Che do cho node trong qua trinh doi phan hoi node tu service ----------------------------
