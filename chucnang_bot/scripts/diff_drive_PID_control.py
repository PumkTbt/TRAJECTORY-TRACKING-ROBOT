...
class MinimalPublisher(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Twist, '/diff_cont/cmd_vel_unstamped', 30)
        self.odom_sub = self.create_subscription(Odometry, "/odom", self.set_pose, 20)
        self.odom_sub

        self.Ts = 0.03 # Thoi gian lay mau 
        self.n = 0 # Bien dem so luong mau
        ...
        self.v = []
        self.w = []
        self.refPose = np.array([0.860,1.998,np.pi/2])
        self.kp = 5
        self.ki = 3.23
        self.kd = 0.5
        self.e_sum = 0
...
#_____________ Dieu khien di theo quy dao ___________________________________________________________
        ...
        if(self.odom_data is not None):
            if (self.position == 0):
                ...
                if abs(ePhi) < self.dTol:
                    ...
                    self.t = np.linspace(0, self.time_utilized, self.n)
