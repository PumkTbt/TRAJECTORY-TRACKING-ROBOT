...
class MinimalPublisher(Node):
    def __init__(self):
        ...
        freq = 2*np.pi/30
        self.xRef = 0.0 + 0.5*np.sin(freq*t)
        self.yRef = 0.0 + 0.5*np.sin(2*freq*t)
        dxRef = freq*0.5*np.cos(freq*t)
        dyRef = 2*freq*0.5*np.cos(2*freq*t)
        ddxRef =-freq**2*0.5*np.sin(freq*t) 
        ddyRef =-4*freq**2*0.5*np.sin(2*freq*t)
